...
Shared renderer CSS is available under `src/renderer/css`. Consuming apps should define their own `--color-app-*`, `--color-panel-*`, and `--color-background` theme tokens before relying on the shared base, component, and keyboard styles. Link `src/renderer/css/fonts.css` directly from the app HTML instead of importing it into Tailwind input CSS, so font URLs remain relative to the package.
